...
describe('Controllers > Orders', () => {
  ...
  it('should forward an error when service.listOrder fails', async () => {
    ...
    const res = buildRes();
    const next = buildNext();
    const error = buildError(StatusCodes.INTERNAL_SERVER_ERROR, 'Error');
  ...
  it('should build a list of errors', () => {
    ...
    jest.spyOn(validator, 'body').mockReturnValueOnce({ exists: existsFn });
  ...
  it('should thrown an error when an unknown method is provided', () => {
    expect(() => {
  ...
  it('should forward and error when service.saveOrder fails', async () => {
    const products = buildOrder();
    const req = buildReq({
      body: { products },
    });
  ...
  it('should return validation response when error bag is empty', async () => {
    const req = buildReq();
    const res = buildRes();
    const next = buildNext();
    const errorBag = {
